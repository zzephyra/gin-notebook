...
import { CreateCategory } from "@/features/api/note";
...
import { CreateNoteData } from "@/features/api/type";
import { forwardRef, useImperativeHandle, useState } from "react";
// ...其余 import 保持不变

export type NoteDropdownRef = {
    openNewNote: () => void;
    openNewCategory: () => void;
};

type Props = { onCreate?: (data: CreateNoteData) => void };

const NoteDropdown = forwardRef<NoteDropdownRef, Props>(({ onCreate }, ref) => {
    const { t } = useLingui();
    const [loadingCategory, setLoadingCategory] = useState(false);
    const [loadingNote, setLoadingNote] = useState(false); // ✅ 原来丢弃了读取端
    ...
    useImperativeHandle(ref, () => ({
        openNewNote: onOpenChangeNewNoteModel,
        openNewCategory: onOpenChangeNewCategoryModel,
    }), [onOpenChangeNewNoteModel, onOpenChangeNewCategoryModel]);
    ...
    async function handleSubmitNewCategory(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const res = await CreateCategory({ category_name: categoryName, workspace_id: params.id });
            if (res.code === responseCode.SUCCESS) onOpenChangeNewCategoryModel();
        } finally {
    ...
    async function handleSubmitNewNote(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            onCreate?.({ ...data, workspace_id: params.id });
            // onOpenChangeNewNoteModel(); // 可选：提交成功后关闭
        } finally {
        ...
        <>
            <Dropdown>
                ...
                <DropdownMenu aria-label="Dropdown Variants" color="default" variant="faded">
                    <DropdownItem key="file" startContent={<DocumentTextIcon className="size-4" />} onPress={onOpenNewNoteModel}>
                        {t`New file`}
                    </DropdownItem>
                    <DropdownItem key="folder" startContent={<FolderPlusIcon className="size-4" />} onPress={onOpenNewCategoryModel}>
                        {t`New folder`}
                    </DropdownItem>
                </DropdownMenu>
            </Dropdown>
                    ...
                    {(onClose) => (
                        <Form onSubmit={handleSubmitNewCategory}>
                            ...
                            <ModalFooter className="w-full">
                                <Button size="sm" onPress={onClose}>{t`Cancel`}</Button>
                                <Button size="sm" color="primary" type="submit" isLoading={loadingCategory}>{t`Create`}</Button>
                            </ModalFooter>
                        </Form>
                    )}
                </ModalContent>
                        ...
                        <Form onSubmit={handleSubmitNewNote}>
                            ...
                            <ModalBody className="w-full gap-6">
                                <Input
                                    ...
                                    errorMessage={t`Note title is required`}
                                    size="sm"
                                    isRequired
                                    label={t`Note Name`}
                                    name="title"
                                    placeholder={t`The note title is required and must not exceed 100 characters in length.`}
                                />
                                <div className="flex gap-4">
                                    <Select
                                        ...
                                        isRequired
                                        className="mt-4"
                                        // ✅ 需要 Iterable<Selection>
                                        defaultSelectedKeys={categoryList[0]?.id ? [String(categoryList[0].id)] : []}
                                    >
                                        {categoryList.map((c) => (
                                ...
                                <fieldset className="border p-4 rounded space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2">
                                        <div className="flex items-center space-x-2">
                                            <Checkbox name="allow_edit" defaultSelected>{t`Allow editing`}</Checkbox>
                                        </div>
                                        <div className="flex items-center space-x-2">
                                            <Checkbox name="allow_comment" defaultSelected>{t`Allow commenting`}</Checkbox>
                                        </div>
                                        <div className="flex items-center space-x-2">
                                            <Checkbox name="allow_share" defaultSelected>{t`Allow sharing`}</Checkbox>
                                        </div>
                                    </div>
                                </fieldset>
                                ...
                                <fieldset className="space-y-3">
                                    ...
                                    >
                                        <Play className={`transition-transform duration-100 ease-in-out size-4 ${showAdvanced ? "rotate-90" : "rotate-0"}`} />
                                        {t`Advanced Collaboration Settings`}
                                    </legend>
                                    ...
                                    <AnimatePresence initial={false}>
                                        ...
                                        >
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 mt-2">
                                                <div className="flex items-center space-x-2">
                                                    <Checkbox name="allow_join" defaultSelected>{t`Allow joining`}</Checkbox>
                                                </div>
                                                <div className="flex items-center space-x-2">
                                                    <Checkbox name="allow_invite" defaultSelected>{t`Allow inviting`}</Checkbox>
                                                </div>
                                            </div>
                                        </motion.div>
                                    </AnimatePresence>
                                ...
                                <div>
                                    <Select
                                        ...
                                        name="status"
                                        label={t`Status`}
                                        placeholder={t`Select note status`}
                                        size="sm"
                                        className="mt-4"
                            ...
                            <ModalFooter className="w-full">
                                <Button size="sm" onPress={onClose}>{t`Cancel`}</Button>
                                <Button size="sm" color="primary" type="submit" isLoading={loadingNote}>{t`Create`}</Button>
                            </ModalFooter>
                        </Form>
                    )}
                </ModalContent>
